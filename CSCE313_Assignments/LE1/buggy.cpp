    ...
    public:
        Shape (int _vertices) {
            ...
            for (int i = 0; i <= vertices; i++){
                points[i] = new Point; //allocating memory for each individual element in the dynamic array
            }
        }

        ~Shape () {
            for (int i = 0; i <= vertices; i++){ //freeing the space that was allocated before in the constructor
                delete points[i];
            }
            delete[] points;
        }
        ...
        double area () { // changed from double* to double as the address of the local variable is going to be cleared once we return
            int temp = 0;
            for (int i = 0; i < vertices; i++) {
...
int main () {
    ...
    //deallocating memory that was allaocated in the main function
    delete tri;
    delete quad;
