...
int main(int argc, char* argv[])
{
        ...
        for(const int64_t& t: trans_arr){
                //looping over every t in trans_arr and creating a thread with the teller_thread handler
                t_vec.push_back(thread(teller_thread, &B, t));
        }
        ...
        for(const int64_t& t: trans_arr){
                //looping over every t in trans_arr and creating a thread with the teller_threadsafe handler
                t_vec2.push_back(thread(teller_threadsafe, &C, t));
        }
        // TODO #5: join all threads in t_vec2
        for(std::thread& t: t_vec2){
                //joining all threads in t_vec2
                t.join();
        }
